...
class MyButtonsApp(App):
    ...
    def show_popup(self, instance, btn_name):
        if btn_name == "Vorbereitung Drohnenführerschein":
            ...
            if content is not None:
                popup = Popup(title=title, content=content, size_hint=(None, None), size=(800, 600))
    ...
    def create_hydrojet_info(self):
        hydrojet_info_layout = GridLayout(cols=1, spacing=12, size_hint_y=None, pos_hint={'top': 0.9})

        hydrojet_info_text = """
                HydroJet Irrigation Systems PLC (HydroJet), headquartered in Kidwelly/UK, is a Welsh
                provider of drone-supported irrigation system solutions, which it offers as Smart Product
                Service Systems (SPSS) for smart agriculture and forestry, winegrowing, vegetable and fruit
                growing, vertical farming and other professional gardening businesses and garden-related
                plants (B2B) as well as for private users (B2C). For more information, please visit our website.
